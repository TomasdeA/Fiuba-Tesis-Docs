...
con tres mecanismos para hacer robusta la detección en condiciones reales de
operación (véase la sección~\ref{subsec:lm-v3-ground-robustness} para la
justificación detallada de cada uno).

El primer filtro descarta toda coordenada $Y < h_{\min}$ ($h_{\min} = 1{,}3\,\text{m}$
por defecto). En el frame alineado, $Y = 0$ es la posición de la cámara e $Y$
aumenta hacia abajo; la cámara se monta en la cabeza del usuario, por lo que
el suelo siempre queda a $Y \geq h_{\min}$ (altura mínima esperada del
usuario). Los puntos por encima de ese umbral corresponden a obstáculos o al
cuerpo del usuario y no constituyen candidatos válidos.

Sobre el conjunto filtrado $\mathcal{F}$ se construye un histograma de la coordenada
$Y$ con celdas de ancho $\Delta_Y$ (por defecto $0{,}05\,\text{m}$). El bin
más poblado localiza el nivel $Y^*$ de la superficie horizontal dominante.
A partir de ese pico se define la banda de suelo:
\begin{equation}
  \label{eq:floor-band}
  \mathcal{B} = \bigl\{i \in \mathcal{F} \;:\; Y^* - \delta_l \leq Y_i \leq Y^* + \delta_h\bigr\}
\end{equation}
donde $\delta_l = 0{,}10\,\text{m}$ y $\delta_h = 0{,}20\,\text{m}$ son márgenes
configurables que absorben la inclinación leve del suelo y el ruido del sensor.
El muestreo de las hipótesis de RANSAC se realiza exclusivamente sobre $\mathcal{B}$,
lo que concentra la búsqueda en la zona más probable del suelo.

El conteo de inliers emplea una tolerancia por punto:
\begin{equation}
  \label{eq:variable-tol}
  \varepsilon_i = \max\bigl(\varepsilon,\; k \cdot \lVert \mathbf{p}_i \rVert\bigr)
\end{equation}
donde $\varepsilon$ es la tolerancia global de la Etapa~3 y $k = 0{,}015$
(parámetro \texttt{floor\_noise\_k}). Los inliers se evalúan únicamente sobre $\mathcal{B}$
y la métrica de calidad se define como:
\begin{equation}
  \label{eq:relative-quality}
  \rho = \frac{|\text{inliers} \cap \mathcal{B}|}{|\mathcal{B}|}
\end{equation}
Este denominador relativo a la banda, en lugar del total de puntos de la nube,
hace que el umbral $\rho_{\min}$ se interprete como fracción del suelo
detectado respecto al suelo candidato, independientemente de cuántos puntos
no relacionados con el suelo haya en la escena.

La primera fase emplea el $70\,$\% de las iteraciones muestreando desde $\mathcal{B}$.
Si $\rho \geq \rho_{\min}$ (por defecto $0{,}3$), se transita a la Etapa~5.
En caso contrario, la segunda fase amplía el pool de muestreo al conjunto
completo $\mathcal{F}$ con las iteraciones restantes, lo que cubre escenarios
en que la banda del histograma no representó bien el suelo.
...
sección~\ref{subsec:lm-v3-ls}.

La razón por la que esta etapa puede operar sobre la nube completa, a
diferencia de la Etapa~4, reside en la diferencia de complejidad
computacional entre RANSAC y el ajuste por mínimos cuadrados. RANSAC
requiere $K$ iteraciones y en cada una recorre la nube entera para contar
inliers, resultando en un costo $O(K \cdot n)$; con $K = 100$ y
$n \approx 300\,000$ puntos, eso supone del orden de $3 \times 10^7$
evaluaciones de distancia por fotograma. El ajuste por mínimos cuadrados,
en cambio, consiste en una única pasada sobre los inliers para acumular la
matriz de covarianza $3 \times 3$ (ecuación~\ref{eq:covariance}), con costo
$O(n_{\mathrm{inl}})$; incluso con decenas de miles de inliers, esa pasada
tarda menos de $2\,\text{ms}$ según las mediciones de la Etapa~8.

Utilizar la nube completa en el refinamiento produce además un plano
con menor sesgo geométrico que si se usara el subconjunto voxelizado.
El voxel grid reemplaza grupos de puntos por su centroide de celda; si
la distribución de puntos dentro de cada celda no es simétrica respecto
al plano real (lo que ocurre frecuentemente cerca de los bordes y en
superficies con variación de profundidad), los centroides introducen un
sesgo sistemático en la estimación. Al ajustar sobre las
medidas originales, ese sesgo desaparece y el estimador converge a la
solución de mínima varianza.
...
\subsubsection{Optimizaciones derivadas del análisis de performance}
\label{subsec:lm-v3-perf-opt}

La Etapa~8 del pipeline no produce resultados geométricos, sino que
instrumenta las demás etapas con marcas de tiempo obtenidas mediante
\texttt{clock\_ge\-ttime(CLOCK\_MO\-NOTONIC)}. El nodo ROS acumula esas
medidas durante diez segundos y emite una línea de log con el promedio y el
máximo de cada etapa, lo que permite identificar cuellos de botella sin
recurrir a herramientas de profiling externas.

Las mediciones realizadas sobre la plataforma de desarrollo (procesador
Intel Core i7, compilando con \texttt{-O2}) revelaron que las dos etapas
iniciales del pipeline concentran prácticamente la totalidad del tiempo de
cómputo: la Etapa~1 (búsqueda de vecinos más cercanos) consume entre
$3{,}5$ y $5{,}5\,\text{ms}$, mientras que la Etapa~2 (construcción del voxel
grid) consume entre $5{,}0$ y $7{,}5\,\text{ms}$. Las etapas restantes
(RANSAC y refinamiento por mínimos cuadrados) resultan significativamente
más rápidas, con tiempos promedio inferiores a $2\,\text{ms}$ cada una.
En consecuencia, el tiempo total del pipeline ronda los $12$\,--\,$18\,\text{ms}$
por fotograma.

El análisis de dependencias de datos del pipeline revela que la Etapa~1 y
la Etapa~2 son completamente independientes entre sí: ambas reciben
exclusivamente la nube de entrada \texttt{cloud} como argumento de solo
lectura, sin acceder a ningún estado mutable compartido. La Etapa~3 requiere
$\sigma_{NN}$ producido por la Etapa~1, y la Etapa~4 requiere la nube
voxelizada producida por la Etapa~2. Por tanto, existe una dependencia de
datos hacia adelante: las dos primeras etapas deben completarse
\emph{entre sí} antes de que pueda iniciarse la Etapa~4, pero no existe
ninguna dependencia entre ellas. Este patrón corresponde a una
bifurcación-unión (\emph{fork-join}), que es la estructura canónica de
paralelismo de datos descrita por Herlihy y Shavit \cite{herlihy2012art}.

La independencia entre la Etapa~1 y la Etapa~2 permite ejecutarlas
concurrentemente. La implementación utiliza \texttt{std::async} con política
\texttt{std::launch::async} de la biblioteca estándar de C++17
\cite{stroustrup2013cpp}, que garantiza la creación de un hilo de sistema
operativo independiente para cada tarea. Cada hilo ejecuta la función
correspondiente sobre la misma nube (acceso de solo lectura, sin condición de
carrera) y deposita su resultado en un objeto \texttt{std::future}. El hilo
principal sincroniza ambas tareas mediante \texttt{future::get()} antes de
continuar con la Etapa~3, consumiendo los dos resultados de forma segura.

El tiempo de pared esperado para el par de etapas paralelas es, en el
mejor caso, $\max(t_1, t_2)$ en vez de $t_1 + t_2$, lo que reduce el
aporte de las dos etapas dominantes de aproximadamente $10\,\text{ms}$ a
aproximadamente $6\,\text{ms}$. La ganancia efectiva depende de la
disponibilidad de núcleos: la plataforma destino (Raspberry Pi~5) cuenta
con cuatro núcleos ARM Cortex-A76, de modo que los dos hilos adicionales
no compiten con el hilo principal de ROS.

Adicionalmente, se introdujo un filtro previo al voxel grid: los puntos con
$Y_i < -\delta_{\text{techo}}$ se descartan antes de la construcción de la
tabla hash. Estos puntos corresponden a la clase \texttt{CEILING} y son
igualmente descartados al final del pipeline en la Etapa~7, por lo que su
exclusión anticipada es semánticamente transparente. La reducción en la
entrada al voxel grid es modesta cuando la cámara apunta hacia el frente,
pero se vuelve significativa cuando el usuario inclina la cabeza hacia abajo
o cuando el techo es bajo, escenarios en los que la nube puede contener
entre el $10\,$\% y el $30\,$\% de puntos de tipo \texttt{CEILING}.

\subsubsection{Robustez ante suelo parcialmente visible}
\label{subsec:lm-v3-ground-robustness}

\paragraph{Problema observado.}
Durante las pruebas con la cámara en posición de uso real (montada en la
cabeza del usuario y apuntando al frente), la detección del plano del suelo
fallaba sistemáticamente a pesar de que el suelo era visible en la nube de
puntos. El diagnóstico reveló dos causas independientes.

La primera causa es estructural: cuando la cámara apunta hacia el frente,
el suelo ocupa una fracción pequeña del campo de visión, típicamente entre
el $5\,$\% y el $8\,$\% del total de puntos. Con la métrica original
$\rho = |\text{inliers}| / n_{\text{total}}$ y umbral $\rho_{\min} = 0{,}3$,
tres décimas partes de la nube deberían ser inliers del plano del suelo para
que la detección prosperara, condición geométricamente imposible cuando el
suelo representa sólo una pequeña fracción de la escena. El algoritmo
rechazaba el plano correcto en la Etapa~5 aunque RANSAC hubiera ajustado
correctamente la superficie.

La segunda causa es metrológica: la tolerancia global $\varepsilon = 3\,\sigma_{NN}$
se calcula a partir de la distribución de distancias al vecino más cercano
sobre la nube completa, dominada por puntos cercanos a la cámara. La
cámara D435 exhibe un error axial que crece con la distancia al cuadrado en
condiciones generales \cite{RS-D400}, y cuya componente perpendicular al
suelo escala aproximadamente como $k \cdot \lVert\mathbf{p}\rVert$ para
superficies aproximadamente horizontales \cite{IntelRealSenseDepthTuning}.
Para puntos del suelo a $2\, \text{m}$--$3\,\text{m}$ de la cámara, la
tolerancia global resultaba demasiado estrecha, causando que esos puntos
fueran descartados como outliers aun perteneciendo al plano.

\paragraph{Solución implementada.}
Se introdujeron tres mecanismos complementarios, descritos formalmente en
la Etapa~4 del pipeline (ecuaciones~\ref{eq:floor-band},
\ref{eq:variable-tol} y~\ref{eq:relative-quality}).

El histograma de $Y$ localiza el nivel del suelo sin asumir que el suelo
domina la nube: el pico del histograma es el nivel más frecuente en la
franja de alturas esperadas, y la banda $\mathcal{B}$ concentra el muestreo
en esa zona. La calidad relativa $\rho = |\text{inliers} \cap \mathcal{B}| / |\mathcal{B}|$
desacopla el umbral de la fracción que el suelo representa en la nube total;
con $\rho_{\min} = 0{,}3$, basta que el $30\,$\% de los candidatos a suelo
sean consistentes con el plano ajustado, independientemente de cuántas
paredes u obstáculos estén presentes. Por último, la tolerancia variable
$\varepsilon_i = \max(\varepsilon, k\cdot\lVert\mathbf{p}_i\rVert)$ adapta
el criterio de inlier al nivel de ruido esperado en cada punto según su
distancia, sin requerir ningún parámetro de calibración adicional más allá
del factor $k$.

\paragraph{Mejora observada.}
Tras introducir los tres mecanismos, la detección del plano del suelo opera
correctamente con la cámara apuntando al frente en entornos de interior
típicos. La figura~\ref{fig:lm-ground-robustness} muestra una comparación
directa del comportamiento antes y después de la modificación: a la
izquierda, la clasificación falla y todos los puntos del suelo son etiquetados
como obstáculos; a la derecha, el plano es detectado correctamente y los
puntos del suelo reciben la etiqueta \texttt{GROUND}.

\begin{figure}[htbp]
  \centering
  % Reemplazar con \includegraphics[width=\linewidth]{fig/ground-detection-comparison.png}
  % cuando la imagen esté disponible.
  \fbox{\parbox{0.9\linewidth}{\centering\vspace{2cm}Imagen: antes/después de la mejora de detección del suelo\vspace{2cm}}}
  \caption{Comparación de la clasificación antes (izquierda) y después (derecha)
           de incorporar el histograma de $Y$, la calidad relativa a la banda y
           la tolerancia variable $\varepsilon_i = \max(\varepsilon, k\cdot\lVert\mathbf{p}_i\rVert)$.
           La cámara apunta hacia el frente; el suelo ocupa menos del $10\,$\% de la nube.
           A la izquierda, todos los puntos del suelo se clasifican erróneamente como
           \texttt{OBSTACLE}. A la derecha, el plano es detectado y los puntos
           del suelo reciben la etiqueta \texttt{GROUND} (verde).}
  \label{fig:lm-ground-robustness}
\end{figure}
